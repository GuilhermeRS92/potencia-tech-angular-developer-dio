  ...
  async checkResult(answers: string[]){
    const result = answers.reduce((prev, curr, i, arr) => {
      if(
        ...
        arr.filter(item => item === curr).length  //cria um filtro dos itens que são iguais e ve o tamanho do array e verifica se o atual é maior que o anterior
        ){
          return prev
      }else{
        return curr
